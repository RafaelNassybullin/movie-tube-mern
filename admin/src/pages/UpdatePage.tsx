...
import { updateStart, getUpadateDataByIdStart } from "store/reducer/adminSlice";
import { updateDataReselect } from "store/selector/adminSelector";
...
export const UpdatePage = () => {
  ...
  const { id } = useParams();
  const dataForUpdate:any = useSelector(updateDataReselect);
  ...
  useEffect(() => {
    setValue("title", dataForUpdate.title);
    setValue("description", dataForUpdate.description);
    setValue("poster", dataForUpdate.poster);
    setValue("urlvideo", dataForUpdate.urlvideo);
    setValue("metatags", dataForUpdate.metatags);
    setValue("category", dataForUpdate.category);
    setValue("duration", dataForUpdate.duration);
    setValue("views", dataForUpdate.views);
  }, [dataForUpdate]);

  useEffect(() => {
    dispatch(getUpadateDataByIdStart(id));
  }, []);

  const submitData = (data: any): void => {
    dispatch(updateStart({ id, data }));
    navigate("/");
  };
...
const Text = styled.p<IStyle>`
  color: ${(props) => (props.colors ? "hotpink" : "crimson")};
  margin-top: 20px;
  margin-left: 15px;
  font-size: 29px;
`;
